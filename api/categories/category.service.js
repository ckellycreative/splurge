...
async function getAllWithTotalByDate(startDate, endDate, userId) {
    const categories = await db.sequelize.query(
        "SELECT categories.id, categories.accountId, categories.category_title, categories.category_type, categories.parentId, ChildCategory.id as `ChildCategory.id`, ChildCategory.accountId as `ChildCategory.accountId`, ChildCategory.category_title as `ChildCategory.category_title`, ChildCategory.category_type as `ChildCategory.category_type`, ChildCategory.parentId as `ChildCategory.parentId`,  ChildCategory.hidden as `ChildCategory.hidden`, totalReportAmountDebit, totalReportAmountCredit, p.id as `CategoryPlan.id`, planAmount as `CategoryPlan.planAmount` FROM categories AS categories LEFT OUTER JOIN categories AS ChildCategory ON categories.id = ChildCategory.parentId LEFT OUTER JOIN ( SELECT transactions.categoryId, SUM(transactions.credit) as totalReportAmountCredit, SUM(transactions.debit) as totalReportAmountDebit FROM transactions WHERE transactions.transaction_date BETWEEN ? AND ? AND transactions.accountId=? GROUP BY transactions.categoryId ) AS t ON ChildCategory.id = t.categoryId LEFT OUTER JOIN ( SELECT plans.id, plans.planAmount, plans.categoryId FROM plans ) AS p ON ChildCategory.id = p.categoryId WHERE categories.accountId = ? AND categories.parentId IS NULL ORDER BY categories.category_type, categories.category_title, ChildCategory.category_title ASC;",
         
         { 
            replacements: [startDate, endDate, userId, userId],
